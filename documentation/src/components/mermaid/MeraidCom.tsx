...
const MermaidCom: FC<MermaidComProps> = ({ fallback, ...props }) => (
  <BrowserOnly fallback={fallback ?? <span className={skeleton()} />}>
    {() => <MermaidContent {...props} />}
  </BrowserOnly>
)

export default MermaidCom

export interface MermaidComProps
  extends
    Pick<ComponentProps<typeof BrowserOnly>, 'fallback'>,
    Pick<MermaidType.MermaidConfig, 'theme'> {
  value: string
  className?: string
